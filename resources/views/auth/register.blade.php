...
<body class="flex items-center justify-center min-h-screen bg-cover bg-center background">
  ...
  <div class="awe rounded-lg shadow-lg p-8 w-full max-w-sm mx-auto">
    ...
    <div class="flex flex-col items-center">
      ...
      <h1 class="text-2xl font-bold gradient-text mb-6">Sign Up</h1>
    </div>
    ...
    <form method="POST" action="{{ route('register') }}">
      ...
      <!-- Name Input -->
      <label for="name" class="block text-sm font-bold gradient-text mb-1">Name</label>
      <div class="relative mb-4">
        <x-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
        <span class="absolute inset-y-0 right-4 flex items-center pointer-events-none">
          <img id="name-icon" src="{{ asset('svg/user-account-profile-svgrepo-com.svg') }}" class="w-5 h-5 text-gray-400" alt="Name">
        </span>
      </div>
      ...
      <!-- Submit Button -->
      <button type="submit" class="w-full px-4 py-2 gradient-bg text-white rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-opacity-50">Register</button>

      <!-- Already have an account? Login Link -->
      @if (Route::has('login'))
        <div class="mt-4 text-center">
        <span class="text-sm text-blue-800">Already have an account? </span><a href="{{ route('login') }}" class="text-sm text-blue-600 hover:underline">Login</a>
        </div>
      @endif
    </form>
  </div>
